...
export const loader = async ({ request }) => {
  ...
  const largeBanners = await prisma.app_banner.findMany({
    where: {
      shop: session.shop,
      size: "large",
  ...
  return {
    ...
    largeBanners,
    totals: {
      enabled: largeBanners.filter((banner) => getBannerStatusMeta(banner).label === "Enabled").length,
      total: largeBanners.length,
    },
  };
};
...
function LargeRow({ banner, navigate, shop }) {
  const fetcher = useFetcher();
...
export default function LargeHomePage() {
  const navigate = useNavigate();
  const { shop, largeBanners, totals } = useLoaderData();

  return (
    <Page title="Large widget" subtitle="Create and manage banners created from the Large layout." backAction={{ content: "Back", onAction: () => navigate("/app") }}>
      <BlockStack gap="500">
        <Card>
          <BlockStack gap="300">
            ...
            <Text as="p" tone="subdued">
              Large is a wide hero banner intended for high-visibility campaigns and landing sections.
            </Text>
            <InlineStack gap="300">
          ...
          {largeBanners.length === 0 ? (
            ...
            <div>
              {largeBanners.map((banner) => (
                <LargeRow key={banner.id} banner={banner} navigate={navigate} shop={shop} />
              ))}
            </div>
          )}
        </Card>
